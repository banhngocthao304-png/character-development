...
function FoodOptionsSection({ categories, run }: { categories: CategoryWithOptions[]; run: Runner }) {
  ...
  const [deleteState, setDeleteState] = useState<DeleteState>(null);
  const moveCategory = (index: number, offset: number) => { const other = categories[index + offset]; if (other) void run(() => reorderCategories(categories[index], other), "Category order updated"); };
  return <>
